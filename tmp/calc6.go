...
const (
	...
	BASE_DATA_VALUE_ROWS = 2 + 2 + 1 + 6 + 6
)
...
func getValues(pos Value) (ret CalcData) {
	...
	for i = 0; i < 2; i++ {
		...
		ret.Values[i][3] = data.Gf[pos+i]
		ret.Values[i][4] = data.Zg2[pos+i]

		for j = 0; j < 6; j++ {
			ret.Values[i][5+j] = data.Gzr[j][pos+i]
		}

		for j = 0; j < 6; j++ {
			ret.Values[i][11+j] = data.Gfr[j][pos+i]
		}
